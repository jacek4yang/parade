...
| NAT/source-address confusion | Conflating hosts, false topology, or enrollment denial | Server-bound Agent credentials—not IPs—are identity. Enrollment uses a broad shared-egress ceiling plus a per-token retry ceiling. Topology returns only bounded source categories/shared counts and verified Agent→Hub edges; it never claims NAT certainty, peer links, public inbound reachability, or exposes raw source IPs by default. |
| Alert/finding flood | Operator overload and database growth | Findings upsert by stable normalized series and count recurrences. Subject-specific rules retain 32 series per server/rule/version plus one overflow series; events and detail rollups have retention. External notification delivery is not in this milestone, so there is no network alert queue to exhaust. |
